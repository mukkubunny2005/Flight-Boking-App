...
const Admin = () => {
  ...
  const fetchData = async () =>{
    await axios.get('http://localhost:5005/fetch-users').then(
      (response)=>{
    ...
    await axios.get('http://localhost:5005/fetch-bookings').then(
      (response)=>{
        setbookingCount(response.data.length);
      }
    );
    await axios.get('http://localhost:5005/fetch-flights').then(
      (response)=>{
  ...
  const approveRequest = async (id) =>{
      try{
          ...
          await axios.post('http://localhost:5005/approve-operator', {id}).then(
            (response)=>{
  ...
  const rejectRequest = async (id) =>{
    try{
      ...
      await axios.post('http://localhost:5005/reject-operator', {id}).then(
        (response)=>{
